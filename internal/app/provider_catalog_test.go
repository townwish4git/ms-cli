...
func TestProviderCatalogCanonicalizesMoonshotLabel(t *testing.T) {
	server := newModelsDevTestServer(`{
		"moonshotai": {
			"id": "moonshotai",
			"name": "Moonshot AI",
	...
	provider, ok := catalog.Provider("moonshotai")
	if !ok {
		t.Fatal("expected moonshotai in catalog")
	}
	if got, want := provider.Label, "Kimi for Coding"; got != want {
		t.Fatalf("provider.Label = %q, want %q", got, want)
	}
}
